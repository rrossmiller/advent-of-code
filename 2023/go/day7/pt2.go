...
func p2(hands []*Hand) {
	// demote J to less powerful than 2
	cardOrder = []string{
		"J",
		"2",
		"3",
	...
	for _, h := range hands {
		...
		// if a joker exists
		if _, exists := counter["J"]; exists {
			// determine card with most occurences, placing priority on highest card
			maxCnt := 0
			bestCard := ""
			for i := len(cardOrder) - 1; i >= 0; i-- {
				if cardOrder[i] == "J" {
					continue
				}
				if v, exists := counter[cardOrder[i]]; exists && v > maxCnt {
					maxCnt = v
					bestCard = cardOrder[i]
				}

			}
			// replace jokers with that card
			h.extraCard = bestCard
			// increment counter for that card
			counter[bestCard] += counter["J"]
			// remove jokers from counter
			delete(counter, "J")
		}
